...
{
         if(word_comparer(command_word, c_ax, wrd_size) == 0) return MY_AX;
    else if(word_comparer(command_word, c_bx, wrd_size) == 0) return MY_BX;
    else if(word_comparer(command_word, c_cx, wrd_size) == 0) return MY_CX;
    else if(word_comparer(command_word, c_dx, wrd_size) == 0) return MY_DX;
    else return REGERR;
}
...
int translator(char* buffer, size_t words_num, FILE* file, size_t size, Lable* lables_array)
{
    ...
    for(size_t i = 0; i <= words_num && index < size; i++)
    {
        ...
        {
            if(!synt_err_check(command_num, __FILE__, __LINE__)) // FIXME words num counter to put lable
                return 0;

            ssize_t number = put_lable(buffer, size, &index, lables_array); // TODO check
            fwrite(&number, sizeof(char), sizeof(char), file);
            i++;
        }
    }
...
{
    char* wrd_begin = find_command_word_begin(buffer, *index, size);
    size_t wrd_len  = lable_len(wrd_begin, *index, size);
    ...
    *index = (size_t)(wrd_begin + wrd_len - buffer);
    ...
    for(size_t i = 0; i < sizeof(lables_array) / sizeof(*lables_array); i++)
    {
        if(strncmp(lables_array[i].name, compare, wrd_len) != 0)
        {
            for(size_t index = 0; index < wrd_len; index++)
            {
                lables_array[i].name[index] = compare[index];
            }
        }
    }

    return ip; // FIXME get ip from words num!
